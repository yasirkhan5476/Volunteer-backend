...
const connection = isServerless ? null : getRedis();

const serverlessQueue = {
  add: async () => ({ id: 'serverless-noop' }),
};

// ─── Queue Definitions ───────────────────────────────────────

/** Email notification queue */
const emailQueue = isServerless
  ? serverlessQueue
  : new Queue('email', {
      connection,
      defaultJobOptions: {
...
const donationReceiptQueue = isServerless
  ? serverlessQueue
  : new Queue('donation-receipt', {
      connection,
      defaultJobOptions: {
        attempts: 3,
        backoff: { type: 'exponential', delay: 5000 },
...
const attendanceQueue = isServerless
  ? serverlessQueue
  : new Queue('attendance', {
      connection,
      defaultJobOptions: {
        attempts: 1,
        removeOnComplete: 50,
